...
msk = cc_4plus_df["sic_ind_occ"].notna() & ~(
    cc_4plus_df["sic_ind_occ"].isin(["", -9, "-9"])
)
...
ID_SUBSET = set(sa_model_df["unique_id"].unique()).intersection(
    cims_df["unique_id"].unique()
)
...
for DIGITS in digits_set:
    ...
    cc_df = prep_clerical_codes(cc_raw_df, cc_4plus_df, digits=DIGITS)

    # prep semantic search model :
    sem_df["top_distance"] = sem_df["semantic_search_results"].apply(
        lambda x: x[0]["distance"] * 1000 if len(x) > 0 else None
    )
    ...
    cims_df["top_distance"] = (1 - cims_df[conf_col]) * 1000
    ...
    combined_df = (
        ...
        .merge(
            sem_df[
                [*info_cols, "semantic_search_results", "top_candidate", "top_distance"]
            ],
            on="unique_id",
            how="outer",
            suffixes=("_CIMS", "_semantic"),
        )
        .copy()
    )
    ...
    for method in ["semantic", "CIMS"]:
        for subset in combined_df["subset"].unique():
            ...
            unambig_msk = eval_df["clerical_codes"].apply(lambda x: len(x) == 1)
            unambig_df = eval_df[unambig_msk].copy().reset_index(drop=True)
            ...
            eval_df = eval_df.drop_duplicates(subset=["top_distance"], keep="last")
